...
### 팀원 C - DB/문서 파싱/결과 문서 생성
**담당 파일**
- `data_layer/repository.py`
- `data_layer/profile_utils.py`
- `data_layer/document_parser.py`
- `data_layer/result_exporter.py`
- `data_layer/seed/`
- `data_layer/runtime_storage/`
...
## 10. 협업 시 충돌 주의 파일
- `backend_api/web_app.py`: 모든 흐름이 모이는 핵심 파일이라 충돌 위험이 높음
- `frontend_ui/static/app.js`: API 변경 시 프론트도 같이 수정되어 충돌이 잦을 수 있음
- `data_layer/repository.py`: DB 스키마 변경 시 전체 기능 영향 큼
- `ai_engine/agent_service.py`: 응답 스키마 변경 시 백엔드/프론트 동시 영향

권장 방식:
- API 스키마 변경은 팀원 A/B/C/D가 먼저 합의
- `autofill_result` 구조를 바꾸면 프론트도 같이 수정
- DB 스키마 변경 시 마이그레이션 방식 먼저 정리

---

## 11. 발표용 한 줄 요약
이 프로젝트는 `로컬 프로필 DB + 문서 파싱 + 로컬 LLM 자동작성 + 결과 문서 다운로드`를 하나의 Flask 웹앱으로 통합한 구조이며, 백엔드/API, AI, 데이터 계층, 프론트엔드로 역할을 분리하기 좋은 형태다.
